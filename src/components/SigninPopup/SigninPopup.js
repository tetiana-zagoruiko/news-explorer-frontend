...
function SigninPopup({ isOpen, onClose, submitSignin }) {
  ...
  const handleSubmit = (e) => {
    ...
    submitSignin(password.current.value, email.current.value);
  }
            ...
            children={
              <React.Fragment>
                ...
                <form action="#" className="form_auth" onSubmit={handleSubmit}>
                  ...
                  <button type="submit" className={`form__save form__save_auth ${buttonClassName}`}>Sign in</button>
                </form>
                <h3 className='popup__text'>or <Link to="/signup" className="popup__link">Sign up</Link></h3>
              </React.Fragment>}
          />
        ) : (
            <div>
